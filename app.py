...
st.set_page_config(page_title="Maior Capital AI Advisor", page_icon="🏠", layout="centered")
...
def apply_bespoke_design():
    st.markdown(f"""
        ...
        .stChatMessage {{ border-radius: 28px; padding: 20px 25px; margin-bottom: 18px; border: 1px solid rgba(255,255,255,0.06) !important; background-color: transparent !important; }}
        .stChatMessage[data-testid="stChatMessageAssistant"] {{ background-color: #111419 !important; border-left: 4px solid {B_GOLD} !important; }}
        ...
        div[data-testid="stChatInput"] {{ border-radius: 35px !important; border: 1px solid rgba(200, 169, 110, 0.2) !important; background-color: #161B22 !important; padding: 10px !important; }}
        .luxury-card {{ background: #181C23; border: 1px solid rgba(200, 169, 110, 0.15); border-radius: 22px; overflow: hidden; transition: all 0.4s ease; margin-bottom: 25px; }}
        ...
        .btn-web {{ text-align: center; background: transparent; border: 1px solid {B_GOLD}; color: {B_GOLD} !important; text-decoration: none; padding: 10px; border-radius: 14px; font-size: 12px; font-weight: 600; }}
...
def call_ai(messages):
    try:
        ...
        return "Service temporarily unavailable. Please contact +39 0789 604545."

apply_bespoke_design()

if "messages" not in st.session_state:
    st.session_state.messages = []
    # Messaggio di benvenuto bilingue per stimolare l'utente
    welcome = "Benvenuto in **Maior Capital**. In quale zona della Sardegna sta cercando la sua proprietà ideale?\n\n*Welcome to Maior Capital. In which area of Sardinia are you looking for your ideal property?*"
...
if prompt := st.chat_input("..."):
    ...
    with st.chat_message("assistant"):
        ...
        found_nums = [int(s.replace('.','')) for s in prompt.replace('€','').split() if s.replace('.','').isdigit() and int(s.replace('.','')) > 10000]
        ...
        if matches:
            # Traduzione dinamica del titolo cards (opzionale, l'AI risponderà comunque nella lingua giusta)
            st.markdown("### 💎 Selection")
            cols = st.columns(len(matches))
            for i, p in enumerate(matches):
                with cols[i]:
                    price = f"€ {p['p_val']:,}".replace(',', '.') if p.get('p_val', 0) > 0 else "Price on Request"
                    wa_msg = f"Interested in: {p['t']} ({p['u']})".replace(" ", "%20")
                    ...
                    st.markdown(f"""
                        <div class="luxury-card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="card-meta">{p.get('l', 'Sardinia')} | {p.get('mq', '---')} MQ</div>
                                <div class="card-title">{p['t'][:50]}...</div>
                                <div class="btn-grid">
                                    <a href="{p['u']}" target="_blank" class="btn-web">VIEW</a>
